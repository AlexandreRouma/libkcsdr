...
int kcsdr_send_command(kcsdr_t* dev, kcsdr_direction_t dir, kcsdr_command_t cmd, const uint8_t* data, int len) {
    ...
    int sent;
    FT_STATUS err = FT_WritePipeEx(dev->ft, 0, (uint8_t*)&pkt, sizeof(kcsdr_packet_t), &sent, NULL);
...
int kcsdr_open(kcsdr_t** dev, const char* serial) {
    ...
    FT_GetChipConfiguration(ft, &config);
...
int kcsdr_set_samplerate(kcsdr_t* dev, kcsdr_direction_t dir, uint32_t samplerate) {
    ...
    return kcsdr_send_command(dev, dir, CMD_SET_PORT, (uint8_t*)&samplerate, 4);
}
...
int kcsdr_rx(kcsdr_t* dev, int16_t* samples, int count) {
    ...
    int received;
    FT_STATUS err = FT_ReadPipeEx(dev->ft, 1, (uint8_t*)samples, count*2*sizeof(uint16_t), &received, NULL);
    return (err == FT_OK) ? received / (2*sizeof(uint16_t)) : -err;
}

int kcsdr_tx(kcsdr_t* dev, const int16_t* samples, int count) {
    // Transmit samples
    int sent;
    FT_STATUS err = FT_WritePipeEx(dev->ft, 1, (uint8_t*)samples, count*2*sizeof(uint16_t), &sent, NULL);
    return (err == FT_OK) ? sent / (2*sizeof(uint16_t)) : -err;
}